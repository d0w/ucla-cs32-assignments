#include <string>
#include <iostream> // TODO: Remove this and main method
#include <cassert>
...
int main() {
	...
	int arr[] = { 2,4,8 };
	int eArr[5];
	assert(totalEclipseOfTheHeart(arr, 3, 10));
	assert(totalEclipseOfTheHeart(arr, 3, 6));
	assert(!totalEclipseOfTheHeart(arr, 3, 11));
	assert(totalEclipseOfTheHeart(arr, 3, 0));
	assert(totalEclipseOfTheHeart(eArr, 0, 0));
	...
	assert(seasonFinale(maze1, 10, 10, 6, 4, 1, 1));
	...
	assert(!seasonFinale(maze2, 10, 10, 6, 4, 8, 1));
