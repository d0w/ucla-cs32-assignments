...
int shuffleHelper(string word, string results[], int i, int j, const string dict[], int size) {
    ...
    if (a == b) { // && (finder(dict[i], results, MAXRESULTS) != -1)
        *results = temp;
        x++;
        return x;
    }
...
//int binarySearch(string word, const string dict[], int top, int bot) {
//    // base case, if no element was found
//    if (top > bot)
//        return -1;
//    else {
//        // find mid of recursive call
//        int mid = (top + bot) / 2;
//
//        // test to see if the middle of the current call (dependent on top and bot) is equal, less, or greater than the expected value
//        if (word == dict[mid])
//            return mid;
//        else if (word < dict[mid])
//            return (binarySearch(word, dict, top, mid-1)); // if expected is before middle of dict, shift bottom to be mid-1
//        else if (word > dict[mid])
//            return (binarySearch(word, dict, mid + 1, bot)); // if expected is after middle of dict, shift top to be mid + 1
//    }
//}

int shuffleHelper(string word, const string dict[], int size, string results[], int i);
